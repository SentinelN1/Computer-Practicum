...
def render(t: ti.f32):
    ...
    for fragCoord in ti.grouped(pixels):
        ...
        colour = ts.vec3(0.)
        # colour.rb = uv + 0.5  # простой расчет цвета пикселя в зависимости от координат u, v
        # colour.r = ts.cos(uv[0] * 47)
        # colour.g = ts.sqrt(uv[0]**2 + uv[1]**2)
        # colour.b = ts.sin(uv[1] * 53)

        colour.rgb = (2.0 + ts.cos(40.0 * u) + ts.cos(40.0 * v)) / 4.0

        pixels[fragCoord] = ts.clamp(colour, 0., 1.)  # clamp(col ** (1 / 2.2), 0., 1.)
